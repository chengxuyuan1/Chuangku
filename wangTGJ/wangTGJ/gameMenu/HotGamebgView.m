...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _longHuBtnView.delegate=self;
        //[self addSubview:_longHuBtnView];
        
//        _ziXuanDTBtnView=[[MenuBtnView alloc] initWithFrame:CGRectZero];
//        [_ziXuanDTBtnView.iconBtn setImage:[UIImage imageNamed:@"lobbyicon_multibac"] forState:UIControlStateNormal];
//        _ziXuanDTBtnView.titleLab.text=[HelperHandle getLanguage:@"自选多台"];
//        _ziXuanDTBtnView.titleLab.textColor=[UIColor colorWithRed:208/255.0 green:144/255.0 blue:68/255.0 alpha:1];
//        _ziXuanDTBtnView.numTipLab.text=@"0";
//        _ziXuanDTBtnView.delegate=self;
//        [self addSubview:_ziXuanDTBtnView];
...
-(void)layoutSubviews{
    ...
    _ziXuanDTBtnView.frame=CGRectMake(_longHuBtnView.frame.origin.x+80+10, 10, 80, 100);
    
}
-(void)menuBtnViewDelegateBtnIndex:(NSString *)title{
    
    if (self.delegate&&[self.delegate respondsToSelector:@selector(hotGameViewDelegateIndex:)]) {
        [self.delegate hotGameViewDelegateIndex:title];
    }
