...
- (void)initialization {
    ...
    _pathBackColor = [UIColor blackColor];
    _pathFillColor = [UIColor blackColor];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat valueEndA = _startAngle + (CircleDegreeToRadian(360)-_reduceValue)*(fakeProgress/_progress);  //数值终点位置
    ...
    if (_showProgressText) {
        //画文字
        NSString *currentText = [NSString stringWithFormat:@"%.0f",fakeProgress];
        UIColor *color=[UIColor whiteColor];
        if (fakeProgress==0) {
            currentText = [NSString stringWithFormat:@"%@",[HelperHandle getLanguage:@"结算中"]];
            color=[UIColor colorWithRed:233/255.0 green:199/255.0 blue:128/255.0 alpha:1];
        }
        //段落格式
...
- (void)setProgress:(CGFloat)progress {
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:1 block:^{
        ...
        if (_animationModel == CircleIncreaseSameTime) {
            fakeProgress -= 1;//不同进度动画时间基本相同
        } else {
            fakeProgress -= 1;//进度越大动画时间越长。
        }
